...
class BaseTask(law.Task):
    version = luigi.Parameter(default="dev1",
                              description="version of current workflow")
...
class CampaignTask(BaseTask):
    ...
    config = luigi.Parameter(default="SUSY_1lep_ML",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.campaign_inst = importlib.import_module("config.{}".format(
            self.campaign_name)).campaign

    def store_parts(self):
        parts = (self.analysis_choice, self.campaign_name,
...
class AnalysisTask(CampaignTask):
    ...
    def _import(self, *parts):
        return importlib.import_module(".".join((self.analysis_choice, ) +
                                                parts))
...
class ShiftTask(ConfigTask):
    ...
    shift = luigi.Parameter(default="nominal",
                            significant=False,
                            description="systematic shift to "
    ...
    def modify_param_values(cls, params):
        ...
        if params["shift"] not in config_inst.shifts:
            raise Exception("shift {} unknown to config {}".format(
                params["shift"], config_inst))
...
class DatasetTask(ShiftTask):
    ...
    dataset = luigi.Parameter(default="singleTop",
                              description="the dataset name, default: "
                              "singleTop")
    ...
    def modify_param_values(cls, params):
        ...
        if params["shift"] in cls.shifts or params[
                "shift"] in dataset_inst.info:
            params["effective_shift"] = params["shift"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dataset_info_inst = self.dataset_inst.get_info(
            self.shift_inst.name if self.shift_inst.name in
            self.dataset_inst.info else "nominal")
